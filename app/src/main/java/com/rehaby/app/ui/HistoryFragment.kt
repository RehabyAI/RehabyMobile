...
import com.rehaby.app.databinding.FragmentHistoryBinding
...
class HistoryFragment : Fragment() {
    ...
    private val adapter = HistorySessionsAdapter()
    ...
    private fun loadHistory() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            val list = withContext(Dispatchers.IO) {
                try {
                    val resp = ApiModule.rehabApi.getPatientHistory(patientId)
            ...
            if (list.isEmpty()) {
                b.emptyText.visibility = View.VISIBLE
                b.emptyText.text = getString(R.string.history_empty)
                adapter.submit(emptyList())
            } else {
